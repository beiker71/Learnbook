	...
	$(".crumb2 .icn-print").attr( "title", "Print this page" );
...
function changeLinks(callback){
	...
	var nodes = $("a").not( $("#sidebar-left .main-menu a") ).not($("navbar-inner ul.nav .pull-right a.btn")).not($("#sidebar-left a.expander")).not($("a[target='_blank']")).not($("a[href^='mailto:']")), i = nodes.length;
...
function nextPage(pageIndex){
	...
	if(newPage==ps.length){ alert('You are on the last page! Use the navigation buttons on the left to complete the module.');return;  }
...
function writeNewPageNo(){
	...
	if (document.getElementById('pageNumberHolder')){
		document.getElementById('pageNumberHolder').innerHTML += ('<table id="progressBarTable"><tr><td align=\"left\" id="progressBarCell"><img src=\"images/img/progressBarBG.jpg\" width=\"' + zWidth + '\" height=\"13\" /> </td></tr><tr><td class="pageNumCell">PAGE ' +  (parseFloat(znThisPage) +1) + ' of ' + lastPageNo +'</td></tr></table>');											
	} 
	else { };
  }	
